...
def main(arg_dict):
    ...
    center_rnd_model = rnd.RNDModel(arg_dict, output_size=512)
    ...
    if arg_dict["trained_model_path"]:
        ...
        center_rnd_model.load_state_dict(checkpoint['rnd_model_state_dict'])
        center_model.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    ...
    model_dict = {
        ...
        'rnd_model_state_dict': center_rnd_model.state_dict(),
    ...
    center_rnd_model.share_memory()
    data_queue = mp.Queue()
    signal_queue = mp.Queue()
    summary_queue = mp.Queue()
    
    processes = [] 
    p = mp.Process(target=learner, args=(center_model, center_rnd_model, data_queue, signal_queue, summary_queue, arg_dict))
    p.start()
    processes.append(p)
    for rank in range(arg_dict["num_processes"]):
        if arg_dict["env"] == "11_vs_11_kaggle":
            p = mp.Process(target=actor_self, args=(rank, center_model, center_rnd_model, data_queue, signal_queue, summary_queue, arg_dict))
        else:
...
if __name__ == '__main__':
    ...
    arg_dict = {
        ...
        "gamma" : 0.993,
        "lmbda" : 0.96,
        "alpha" : 0.993,  # discount factor for intrinsic reward
        "ratio" : 0.00001,  # 外部奖励和内部奖励的比例
        "entropy_coef" : 0.0001,
        "grad_clip" : 3.0,
        "eps_clip" : 0.1,
