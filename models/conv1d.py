...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_vi1 = nn.Linear(arg_dict["lstm_size"], 164)
        self.norm_vi1 = nn.LayerNorm(164)
        self.fc_vi2 = nn.Linear(164, 1, bias=False)
    ...
    def forward(self, state_dict):
        ...
        v_e = F.relu(self.norm_v1(self.fc_v1(out)))
        v_e = self.fc_v2(v_e)

        v_i = F.relu(self.norm_vi1(self.fc_vi1(out)))
        v_i = self.fc_vi2(v_i)

        return prob, prob_m, v_e, v_i, h_out
    ...
    def make_batch(self, data):
        ...
        a_batch, m_batch, r_batch, intr_r_batch, prob_batch, done_batch, need_move_batch = [], [], [], [], [], [], []
        ...
        for rollout in data:
            ...
            a_lst, m_lst, r_lst, intr_r_lst, prob_lst, done_lst, need_move_lst = [], [], [], [], [], [], []

            for transition in rollout:
                s, a, m, r, intr_r, s_prime, prob, done, need_move = transition
                ...
                m_lst.append([m])
                r_lst.append([r])
                intr_r_lst.append([float(intr_r)])  # convert ndarray to float
            ...
            intr_r_batch.append(intr_r_lst)
        ...
        # print(r_batch, intr_r_batch)
        # print(type(r_batch), type(intr_r_batch))
        a, m, r, intr_r, done_mask, prob, need_move = torch.tensor(a_batch, device=self.device).permute(1, 0, 2), \
                                              ...
                                              torch.tensor(r_batch, dtype=torch.float, device=self.device).permute(1, 0,
                                                                                                                   2), \
                                              torch.tensor(intr_r_batch, dtype=torch.float, device=self.device).permute(1,0,2), \
        ...
        return s, a, m, r, intr_r, s_prime, done_mask, prob, need_move
