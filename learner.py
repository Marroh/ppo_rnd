...
from utils import normalize_intr_r

def write_summary(writer, arg_dict, summary_queue, n_game, loss_lst, pi_loss_lst, v_loss_lst, v_i_loss_lst, rnd_loss_lst, \
    ...
    writer.add_scalar('train/v_i_loss', np.mean(v_i_loss_lst), n_game)
    writer.add_scalar('train/rnd_loss', np.mean(rnd_loss_lst), n_game)
...
def learner(center_model, center_rnd_model, queue, signal_queue, summary_queue, arg_dict):
    print("Learner process started")
    imported_model = importlib.import_module("models." + arg_dict["model"])  # conv1d
    imported_algo = importlib.import_module("algos." + arg_dict["algorithm"])  # training program of ppo
    ...
    n_game = 0
    loss_lst, pi_loss_lst, v_loss_lst, v_i_loss_lst, rnd_loss_lst, entropy_lst, move_entropy_lst = [], [], [], [], [], [], []
    self_play_board = {}
    ...
    while True:
        if queue.qsize() > arg_dict["batch_size"]*arg_dict["buffer_size"]:
            ...
            mean, std = normalize_intr_r(data, center_rnd_model)
            loss, pi_loss, v_loss, v_i_loss, rnd_loss, entropy, move_entropy = algo.train(model, center_rnd_model, data, mean, std)
            ...
            v_i_loss_lst.append(v_i_loss)
            rnd_loss_lst.append(rnd_loss)
            ...
            if summary_queue.qsize() > arg_dict["summary_game_window"]:
                win_evaluation, score_evaluation = write_summary(writer, arg_dict, summary_queue, n_game, loss_lst, pi_loss_lst, 
                                                                 v_loss_lst, v_i_loss_lst, rnd_loss_lst, entropy_lst, move_entropy_lst, optimization_step,
                                                                 self_play_board, win_evaluation, score_evaluation)
                loss_lst, pi_loss_lst, v_loss_lst, v_i_loss_lst, entropy_lst, move_entropy_lst = [], [], [], [], [], []
